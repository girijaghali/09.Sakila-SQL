...
from sakila.staff st 
inner join sakila.payment pt on st.staff_id = pt.staff_id
where date_format(pt.payment_date, '%Y-%m-%d') > '2005-08-01'
group by 1,2;
...
inner join sakila.country cy on ct.country_id = cy.country_id
...
create view sakila.top_five_genre
as 
(
select c.name , sum(p.amount) gross_revenue
from sakila.inventory i
